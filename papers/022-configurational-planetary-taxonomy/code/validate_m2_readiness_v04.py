...
EV=ROOT/"data"/"evidence_state_v0.9.csv";DER=ROOT/"data"/"derived_physics_v0.2.csv"
def read(p):
...
def main():
    ...
    exp={'scale':34,'material':34,'atmosphere':40,'organization':40,'solar':34,'complete':27};exps={'planet':8,'dwarf_planet':4,'satellite':15,'small_body':0}
    assert cov==exp,(cov,exp);assert strata==exps,(strata,exps)
    assert cov["atmosphere"]>=40 and cov["organization"]>=40
    assert next(r for r in ev if r["name"]=="Umbriel")["atmosphere_value"]=="NONE_OR_NEGLIGIBLE"
    assert next(r for r in ev if r["name"]=="Titania")["atmosphere_value"]=="NONE_OR_NEGLIGIBLE"
    print("PASS: readiness Batch 08",cov,strata)
    print("PASS: ATMOSPHERE_RETENTION and INTERNAL_ORGANIZATION coverage gates pass")
    print("PASS: remaining dominant gate is small-body numeric acquisition")
